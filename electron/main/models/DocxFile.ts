...
class DocxFile {
    ...
    async read() {
        ...
        if (xmlFile) {
            this.documentXml = await xmlFile.async('nodebuffer');
            const parser = new DOMParser();
            const parsedXml = parser.parseFromString(this.documentXml.toString(), 'application/xml');
