...
class DocxAsXml {
    ...
    private containsOnlyNumbersAndSpaces = (str) => {
        ...
        for (const node of this.paragraphNodes) {
            ...
            for (const row of rows) {
                ...
                if (fragments.length > 1) {
                    this.select('w:t', row)[0]['textContent'] = fragments[0] + ' ';
                    this.currentBmText += fragments[0];
                    this.closeBmAfter(row);
                    for (let i = 1; i < fragments.length; i++) {
                        this.currentBmText = fragments[i];
                        const clonedRow = row.cloneNode(true);
                        this.select('w:t', clonedRow)[0]['textContent'] =
                            fragments[i] + (i < fragments.length - 1 ? ' ' : '');
